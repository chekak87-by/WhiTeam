      ...
      {/* ЛЕВАЯ КОЛОНКА (Заголовок и текст) */}
           ...
           {/* ПРАВАЯ КОЛОНКА (Премиальная галерея отзывов со свайпами и оригинальными аватарками) */}
              <div className="w-full xl:col-start-7 xl:col-span-6 flex flex-col items-center justify-center relative min-h-[400px] xl:min-h-full mt-10 xl:mt-0 select-none">
                  ...
                  {[0, 1, 2].map((index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="absolute top-6 right-6 md:top-8 md:right-8 text-[10px] md:text-xs font-mono font-medium tracking-widest text-purple-400">
                        ...
                        <div className="flex flex-col justify-between h-full w-full pointer-events-none">
                          ...
                          <div className="flex items-center gap-4 mt-2">
                            ...
                            <div className="flex flex-col gap-2.5 w-[50%]">
                              ...
                              <div className="w-[60%] h-2.5 rounded-full bg-purple-500/80"></div>
                            </div>
                          </div>

                          {/* Текст (Скелетон) */}
                          <div className="flex flex-col gap-3.5 w-full mt-6 mb-auto">
                            <div className="w-[95%] h-2.5 rounded-full bg-zinc-700/80"></div>
                            <div className="w-[85%] h-2.5 rounded-full bg-zinc-700/80"></div>
                            <div className="w-[90%] h-2.5 rounded-full bg-zinc-700/80"></div>
                ...
                {/* Вынесенная вниз панель управления */}
                <div className="w-full max-w-[320px] sm:max-w-[380px] xl:max-w-[420px] flex justify-between items-center mt-6 md:mt-8 relative z-40 px-2">
                  
                  {/* Индикаторы текущего отзыва (Точки) */}
                  <div className="flex gap-2">
                     {[0, 1, 2].map((i) => (
                       <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${i === activeReview ? 'w-6 bg-purple-500 shadow-[0_0_8px_rgba(168,85,247,0.6)]' : 'w-1.5 bg-zinc-700'}`} />
                     ))}
                  </div>
              ...
              <div className="flex justify-center md:justify-end w-full md:w-auto">
                <motion.div initial={{ opacity: 0.3 }} animate={{ opacity: isTypingComplete ? 1 : 0.3 }} transition={{ duration: 0.25, ease: "linear" }} className="text-xs md:text-sm font-mono tracking-wide text-[#71717A] flex items-center pointer-events-auto transform-gpu select-none">
                  Status: <span className={`ml-1.5 font-medium transition-colors duration-300 ${activePage !== 'about' && isTypingComplete ? 'text-purple-500 animate-pulse' : 'text-[#52525B]'}`}>
                    {activePage !== 'about' ? 'online' : 'offline'}
                  </span>
                </motion.div>
              </div>
